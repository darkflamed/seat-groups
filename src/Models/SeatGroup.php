...
class SeatGroup extends Model
{
    ...
    {
        return $this->hasMany('Herpaderpaldent\Seat\SeatGroups\Models\ShipsSeatgroups', 'seatgroup_id');
    }
    ...
    public function isQualified(Group $group)
    {
        ...
        foreach ($group->users as $user){
            ...
            if ($this->matchesCorporationAlliance($affiliations, $user)
                && $this->matchesCorporationTitle($affiliations, $user)
                && $this->matchesSkills($affiliations, $user)
                && $this->matchesShipAndShipLocation($affiliations, $user))
                    return true;
        }
    ...
    private function matchesCorporationAlliance($affiliations, $user)
    {
    ...
    private function matchesCorporationTitle($affiliations, $user)
    {
        $matchTitle = true;
        foreach($affiliations as $affiliation){
    ...
    private function matchesSkills($affiliations, $user)
    {
        $matchSkills = true;
    ...
    private function matchesShipAndShipLocation($affiliations, $user)
    {
        $matchShips = true;
        foreach($affiliations as $affiliation){
            if (isset($affiliation['ship_id']))
                $matchShips = false;
        }
        ...
        foreach($affiliations as $affiliation){
            if (isset($affiliation['ship_id'])) {
                foreach($user->character->assets as $asset) {
                    if($asset->type_id == $affiliation['ship_id'])
                        return true;
                }

                if($user->character->ship->ship_type_id == $affiliation['ship_id']) {
                    return true;
                }
            }
        }
